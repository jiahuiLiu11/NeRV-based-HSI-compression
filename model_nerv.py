...
class CustomDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tensor_image=transform(image).unsqueeze(-1)
...
class Generator(nn.Module):
    def __init__(self, **kargs):
        ...
        print("stem_dim:",stem_dim)
        print("stem_num:",stem_num)
    ...
    def forward(self, input):
        ...
        for layer, head_layer in zip(self.layers, self.head_layers):
            ...
            if head_layer is not None:
                # print("head_input",output.shape)
                img_out = head_layer(output)
                print("im:::",img_out.shape)
                img_out=img_out[:,:,:256,:1476]
